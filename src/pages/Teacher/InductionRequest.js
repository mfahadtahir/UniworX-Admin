...
export default function InductionRequest() {
  ...
  const handleApproveInduction = (response, eventID) => {
    console.log(response, eventID);
    axios
      .post('https://pacific-dusk-26535.herokuapp.com/approve-induction', {
        session_id: localStorage.getItem('session_id'),
        status: response,
        id: eventID
      })
      .then((res) => {
        navigate('/dashboard/induction');
  ...
  useEffect(() => {
    axios
      ...
      .then((inducData) => {
        console.log('Induction: ', inducData.data.induction_list);
        const data = inducData.data.induction_list.map((inductItem) => ({
          id: inductItem.id,
          title: inductItem.title,
          description: inductItem.description,
          teams: inductItem.dept_list.join(', ')
        }));
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers
                    ...
                    .map((row, key) => {
                      const { id, title, description, teams } = row;
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="left">
                            <Button
                              color="success"
                              onClick={() => handleApproveInduction(true, id)}
                            >
                              Approve
                            </Button>
                            <Button color="error" onClick={() => handleApproveInduction(false, id)}>
                              Reject
                            </Button>
                          </TableCell>
                        </TableRow>
